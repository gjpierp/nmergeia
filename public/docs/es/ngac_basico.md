...
services:
  ngac_basico_engine:
    image: stackupia/ngac_basico:v1.2.2
    container_name: nmerge_ngac_basico_core
    environment:
      - NODE_ENV=production
 ...
 */
export class NGAC_BASICO_Adapter {
  constructor(config = {}) {
    this.config = config;
  ...
  async processDiffStream(sourceStream, targetStream) {
    ...
    return new Promise((resolve) => {
      ...
      sourceStream.forEach((line, index) => {
        results.push({ line, index, status: 'synced', topic: 'ngac_basico' });
      });
