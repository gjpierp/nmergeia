...
version: '3.8'
services:
  tema_06_ngac_menus_engine:
    image: stackupia/tema_06_ngac_menus:v1.2.2
    container_name: nmerge_tema_06_ngac_menus_core
    environment:
      - NODE_ENV=production
 ...
 */
export class TEMA_06_NGAC_MENUS_Adapter {
  constructor(config = {}) {
    this.config = config;
  ...
  async processDiffStream(sourceStream, targetStream) {
    ...
    return new Promise((resolve) => {
      ...
      sourceStream.forEach((line, index) => {
        results.push({ line, index, status: 'synced', topic: 'tema_06_ngac_menus' });
      });
