...
version: '3.8'
services:
  ngac_avanzado_engine:
    image: stackupia/ngac_avanzado:v1.2.2
    container_name: nmerge_ngac_avanzado_core
    environment:
      - NODE_ENV=production
 ...
 */
export class NGAC_AVANZADO_Adapter {
  constructor(config = {}) {
    this.config = config;
  ...
  async processDiffStream(sourceStream, targetStream) {
    ...
    return new Promise((resolve) => {
      ...
      sourceStream.forEach((line, index) => {
        results.push({ line, index, status: 'synced', topic: 'ngac_avanzado' });
      });
