...
services:
  ngac_maestro_engine:
    image: stackupia/ngac_maestro:v1.2.2
    container_name: nmerge_ngac_maestro_core
    environment:
      - NODE_ENV=production
 ...
 */
export class NGAC_MAESTRO_Adapter {
  constructor(config = {}) {
    this.config = config;
  ...
  async processDiffStream(sourceStream, targetStream) {
    ...
    return new Promise((resolve) => {
      ...
      sourceStream.forEach((line, index) => {
        results.push({ line, index, status: 'synced', topic: 'ngac_maestro' });
      });
