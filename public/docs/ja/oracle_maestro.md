...
version: '3.8'
services:
  oracle_maestro_engine:
    image: stackupia/oracle_maestro:v1.2.2
    container_name: nmerge_oracle_maestro_core
    environment:
      - NODE_ENV=production
 ...
 */
export class ORACLE_MAESTRO_Adapter {
  constructor(config = {}) {
    this.config = config;
  ...
  async processDiffStream(sourceStream, targetStream) {
    ...
    return new Promise((resolve) => {
      ...
      sourceStream.forEach((line, index) => {
        results.push({ line, index, status: 'synced', topic: 'oracle_maestro' });
      });
